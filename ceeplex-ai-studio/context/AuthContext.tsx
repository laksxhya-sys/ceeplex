...
export const AuthProvider = ({ children }: { children?: ReactNode }) => {
  // Initialize with guest session if available
  const [user, setUser] = useState<User | null>(() => {
    try {
      const saved = localStorage.getItem('ceeplex_guest');
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser && firebaseUser.email) {
        ...
        localStorage.removeItem('ceeplex_guest');
        ...
        // If not logged in via Firebase, check if we are in guest mode
        const guest = localStorage.getItem('ceeplex_guest');
        if (guest) {
  ...
  const logout = async () => {
    try {
      localStorage.removeItem('ceeplex_guest'); // Clear guest session
  ...
  const loginAsGuest = () => {
    ...
    setUser(guestUser);
    localStorage.setItem('ceeplex_guest', JSON.stringify(guestUser));
  };
